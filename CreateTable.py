...
def extract_features(data):
    # Gaussian Smoothing
    data['Fz_s'] = gaussian_filter1d(data['forceZ'], sigma=1)
    data['posz_s'] = gaussian_filter1d(data['posz'], sigma=1)
    ...
    F_ss = np.mean(fz_touch [-20:])

    # Hardness
    stiffness = data['forceZ'].max() / (data['posz'][data['forceZ'].idxmax()])
    stiffness2 = (data['forceZ'].max()- np.mean(fz_touch[-10:])) / (data['posz'][data['forceZ'].idxmax()]- np.mean(pz_touch[-10:]))
    ...
    # Power over different frequency
    ...
    return stiffness,stiffness2,tau,F_ss,power,entropy,psd_peak,upstroke,downstroke, upstroke_fp,downstroke_fp
...
def get_label(posx, posy, centers, radii) -> int:
    for i in range(len(centers)):
        ...
        if ((posx - center_x) ** 2 + (posy - center_y) ** 2) <= (radius-1) ** 2 :
...
def organize_df(df_input: DataFrame, centers, radii) -> DataFrame | None:
    ...
    df_input['forceZ'] = (df_input['forceZ'] - offset) / np.mean(df_input['forceZ'][df_input['isArrived_Festo'] == 1])
    df_input['CPXEts'] = df_input['CPXEts'] - df_input['CPXEts'].min()

    posx = np.mean(df_input['posx'][df_input['isArrived_Festo'] == 1] + df_input['posx_d'][df_input['isArrived_Festo'] == 1] / 1000 ).round()
    posy = np.mean(df_input['posy'][df_input['isArrived_Festo'] == 1] + df_input['posy_d'][df_input['isArrived_Festo'] == 1] / 1000 ).round()
    ...
    stiffness,stiffness2,tau,F_ss,power,entropy,psd_peak,upstroke,downstroke, upstroke_fp,downstroke_fp = extract_features(df_input) #change label
    ...
    new_df = DataFrame({
        ...
        "Upstroke_fp": upstroke_fp,
        "Downstroke_fp": downstroke_fp,
