...
def extract_features(data):
    ...
    # Hardness
    stiffness = data['Fz_s'].max() / (data['posz_s'][data['Fz_s'].idxmax()])
    ...
    prob_dist = np.histogram(data['Fz_s'][data['isTouching_SMAC'] == 1], bins=30, density=True)[0]  # Probability distribution
...
def organize_df(df_input: DataFrame, centers, radii) -> DataFrame | None:
    ...
    stiffness, tau, F_ss, power, entropy, psd_peak, upstroke, downstroke, fi, pi, time_i, P_ss, offset = extract_features(df_input)  # change label
